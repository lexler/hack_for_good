class FinishEvaluationApp {
    ...
    generateEmailContent() {
        ...
        if (this.isSkipCoding) {
            ...
            const countIds = [1, 2, 3, 4, 6, 7, 8, 9];
            countIds.forEach(id => {
                const labelData = this.labels[id];
                const displayLabel = labelData ? `${labelData.code} (${labelData.description})` : `Button ${id}`;
                content += `${displayLabel}: ${this.counts[id] || 0}\n`;
            });
        }

        content += '\n';

        const daysValue = this.questionAnswers.didNotCollect ? 'Not collected' : this.questionAnswers.daysPracticed;
        content += `Number of days practiced last week: ${daysValue}\n`;

        const scoreValue = this.questionAnswers.didNotAdminister ? 'Not collected' : this.questionAnswers.ecbiScore;
        content += `ECBI/WACB score: ${scoreValue}\n`;

        content += `Coached (mins): ${this.questionAnswers.coachingTime}\n`;
        content += '\nAdditional notes:\n';
